...
- Defer feature PRs for remote operation, background automation, dashboards,
  ...
  multi-project orchestration, and package-manager expansion.
...
1. Governance files task: add `LICENSE`, `CONTRIBUTING`, `SECURITY`, and
   ...
   existence, required contact/channel text, first-release scope limits, and no
   package metadata changes unless explicitly included.
2. Package metadata task: update `package.json` only after governance files are
   ...
   packaging-safe check, and a package contents dry run before any package
   publishing claim.
