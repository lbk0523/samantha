...
import { cleanupCompletedWorktree } from "../src/core/worktree-cleanup";
...
async function makeRepo(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "samantha-cleanup-"));
  tmpRoots.push(root);
...
async function makePassedRun(options: { merge: boolean }): Promise<{
  ...
  const logRoot = await mkdtemp(join(tmpdir(), "samantha-cleanup-log-"));
...
describe("cleanupCompletedWorktree", () => {
  ...
  test("classifies already-cleaned worktrees idempotently", async () => {
    const { root, logPath, worktreePath, branch } = await makePassedRun({ merge: true });
